 ...
 * - All calculations in UTC, display in dealer's timezone
 */
@Service
@Slf4j
...
public class DealerTrialService {
    ...
    public boolean isTrialActive(Dealer dealer) {
        if (dealer.getTrialExpired()) {
            return false;
        }

        if (dealer.getSubscriptionTier() != null && !"trial".equals(dealer.getSubscriptionTier())) {
            return false;
        }
    ...
    public boolean canCreateListing(Dealer dealer) {
        // Check if dealer is allowed to create listings (feature flag)
        if (!dealer.getCanCreateListings()) {
            log.warn("Dealer {} blocked from creating listings (feature flag)", dealer.getId());
            return false;
        }
        ...
        if (status.isInGracePeriod()) {
            boolean underLimit = dealer.getTrialListingsCount() < trialListingLimit;
            log.info("Dealer {} in grace period, can create: {}", dealer.getId(), underLimit);
            return underLimit;
        }

        // If has active subscription, check subscription limits
        if (dealer.hasActiveSubscription()) {
            return checkSubscriptionLimit(dealer);
        }

        // Trial expired and no subscription
        log.warn("Dealer {} cannot create listing: trial expired, no subscription", dealer.getId());
        return false;
    }
    ...
    public TrialStatus getTrialStatus(Dealer dealer) {
        ZonedDateTime now = ZonedDateTime.now(ZoneId.of("UTC"));
        ZonedDateTime trialEnd = getTrialEndDate(dealer);
        ZonedDateTime graceEnd = trialEnd.plusDays(gracePeriodDays);

        long daysRemaining = ChronoUnit.DAYS.between(now, trialEnd);
        boolean active = now.isBefore(trialEnd) && !dealer.getTrialExpired();
        ...
        if (canCreate) {
            ...
            boolean withinPeriod = active || inGracePeriod || dealer.hasActiveSubscription();
            // Check if under listing limit
            boolean underLimit = dealer.getTrialListingsCount() < trialListingLimit;
    ...
    public ZonedDateTime getTrialExpiryInDealerTimezone(Dealer dealer) {
        ZonedDateTime trialEndUTC = getTrialEndDate(dealer);
        String dealerTimezone = dealer.getTimezone() != null ? dealer.getTimezone() : "Asia/Damascus";
     ...
     */
    @Transactional
    public void expireTrial(Dealer dealer) {
        dealer.setTrialExpired(true);
        dealerRepository.save(dealer);
        log.info("Expired trial for dealer {}", dealer.getId());
    }
    ...
    private ZonedDateTime getTrialEndDate(Dealer dealer) {
        ...
        if (dealer.getTrialExtendedUntil() != null) {
            return dealer.getTrialExtendedUntil().withZoneSameInstant(ZoneId.of("UTC"));
        }

        return trialStart.plusMonths(trialDurationMonths);
    }
    ...
    public static class TrialStatus {
        ...
        private int listingsUsed;
        private int listingsRemaining;
        private int listingsLimit;
        private double usagePercent;
        private ZonedDateTime expiresAt;
        private boolean inGracePeriod;
        private ZonedDateTime graceEndsAt;
