...
public class CacheConfig implements CachingConfigurer {
    ...
    /**
     * Degrade gracefully when the cache backend is unavailable: a Redis outage
     * must fall back to the database, never fail the request. Without this,
     * every @Cacheable method throws on cache errors (which broke startup when
     * Redis was unreachable — data initializers misread the failure as
     * "row missing" and inserted duplicates).
     */
    @Override
    public CacheErrorHandler errorHandler() {
        return new CacheErrorHandler() {
            ...
            public void handleCacheGetError(RuntimeException e, Cache cache, Object key) {
                log.warn("Cache GET failed on '{}' (key {}), falling back to source: {}",
