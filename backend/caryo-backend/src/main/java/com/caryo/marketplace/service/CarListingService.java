    ...
    private void validateDealerCanCreateListing(String username) {
        ...
        if (dealerService.isDealer(user)) {
            ...
            Dealer dealer = dealerService.getDealerByUserId(user.getId())
                .orElseThrow(() -> new com.caryo.marketplace.exception.dealer.DealerNotFoundException(user.getId()));
            ...
            if (!dealerTrialService.canCreateListing(dealer)) {
                ...
                if ("trial".equals(tier)) {
                    throw new com.caryo.marketplace.exception.dealer.TrialExpiredException(listingsUsed, 15);
                } else if ("suspended".equals(dealer.getSubscriptionStatus())) {
                    throw new com.caryo.marketplace.exception.dealer.TrialExpiredException(tier, true);
                } else {
                    throw new com.caryo.marketplace.exception.dealer.SubscriptionLimitExceededException(tier, listingsUsed, 100);
                }
            }
