...
if [ ! -d "$POSTMAN_DIR" ]; then
    ...
    POSTMAN_DIR="/Users/aliahdab/Documents/Dev/ali/caryo-marketplace/backend/caryo-backend/src/test/resources/postman"
    if [ ! -d "$POSTMAN_DIR" ]; then
...
run_all_collections() {
  ...
  if [[ -f "$POSTMAN_DIR/caryo-api-collection.json" ]]; then
    print_info "Using combined API collection"
    if newman run "$POSTMAN_DIR/caryo-api-collection.json" -e "$ENVIRONMENT_FILE" --color auto; then
