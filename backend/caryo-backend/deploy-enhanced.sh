...
backup_data() {
    ...
    if docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T db pg_dump -U "${DB_USER:-caryo}" -d "${DB_NAME:-caryo}" > "$backup_dir/database.sql"; then
...
restore_data() {
    ...
    if [ -f "$backup_dir/database.sql" ]; then
        ...
        cat "$backup_dir/database.sql" | docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T db psql -U "${DB_USER:-caryo}" -d "${DB_NAME:-caryo}"
...
open_db_shell() {
    ...
    docker compose -f docker-compose.yml -f docker-compose.prod.yml exec db psql -U "${DB_USER:-caryo}" -d "${DB_NAME:-caryo}"
}
