...
backup_data() {
    ...
    docker compose -f docker-compose.yml -f docker-compose.prod.yml exec db pg_dump -U caryo -d caryo > $BACKUP_DIR/database.sql
...
restore_data() {
    ...
    cat $BACKUP_DIR/database.sql | docker compose -f docker-compose.yml -f docker-compose.prod.yml exec -T db psql -U caryo -d caryo
