...
export COMPOSE_PROJECT_NAME="caryo_dev"
...
check_and_restore_images() {
    ...
    mc alias list 2>/dev/null | grep -q "caryo-local" || {
        mc alias set caryo-local http://localhost:${MINIO_API_PORT:-9000} minioadmin minioadmin 2>/dev/null
    }
    
    # Check if sample images exist (check for car-8-1.jpg as a representative)
    if mc stat caryo-local/caryo-assets/sample/car-8-1.jpg &>/dev/null; then
...
start_dev_env() {
    ...
    echo -e "\n${YELLOW}Use './caryo.sh dev health' to check service health${NC}"
}
