...
Use the unified CLI wrapper `caryo.sh` for production operations. These commands delegate to the production deployment script and keep logic centralized.

```bash
# From repo root or backend folder
./backend/caryo-backend/caryo.sh prod deploy          # Build and start prod
./backend/caryo-backend/caryo.sh prod rebuild         # Rebuild images (keep DB)
./backend/caryo-backend/caryo.sh prod clean-rebuild   # Wipe DB/volumes, rebuild from scratch
./backend/caryo-backend/caryo.sh prod backup          # Create DB/uploads/logs backups
./backend/caryo-backend/caryo.sh prod health          # Check service health
```
...
|----------|-------------|---------|
| DB_USER | Database username | caryo |
