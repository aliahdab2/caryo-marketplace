    ...
    <div className={`min-h-screen flex flex-col md:${flexDirection} bg-gradient-to-br from-gray-50 via-white to-gray-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 animate-fade-in`} dir={dir} data-rtl={isRTL ? 'true' : 'false'} data-lang={currentLang}>
      ...
      <div className={`hidden md:flex md:w-2/5 lg:w-1/3 xl:w-1/4 bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 text-white flex-col justify-between relative overflow-hidden shadow-2xl ${isRTL ? 'md:order-2' : 'md:order-1'}`}>
        ...
        <div className={`z-10 p-6 md:p-8 lg:p-10 flex flex-col ${textAlign} animate-slide-in-left`}>
          <div className={`flex items-center mb-8 group ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
            <div className="h-12 w-12 relative flex-shrink-0 transform group-hover:scale-110 transition-transform duration-300">
              <NextImage
                ...
                width={48}
                height={48}
                className={`w-10 h-10 md:w-12 md:h-12 object-contain filter invert drop-shadow-lg ${marginStart('3')} md:${marginStart('4')}`}
                priority
              />
            </div>
      ...
      <div className={`flex-1 flex flex-col justify-center px-4 md:px-6 lg:px-8 xl:px-12 py-6 md:py-8 pb-8 md:pb-12 auth-container ${isRTL ? 'md:order-1' : 'md:order-2'} animate-slide-in-right`}>
