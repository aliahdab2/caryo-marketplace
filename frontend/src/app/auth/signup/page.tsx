...
export default function SignUpPage() {
  ...
  return (
    <div className={`min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 ${direction.className}`} dir={direction.dir}>
      ...
      <div className={`hidden md:flex md:w-2/5 lg:w-1/3 xl:w-1/4 bg-gradient-to-r from-blue-600 to-blue-800 text-white flex-col justify-between relative overflow-hidden ${isRTL ? 'md:order-2' : 'md:order-1'}`}>
        ...
        {/* Content */}
        <div className={`z-10 p-6 md:p-8 lg:p-10 flex flex-col ${direction.textAlign}`}>
          <div className={`flex items-center mb-6 ${direction.flexDirection}`}>
            <div className="h-10 w-10 relative flex-shrink-0">
              <NextImage
                ...
                width={40}
                height={40}
                className={`w-8 h-8 md:w-10 md:h-10 object-contain filter invert ${direction.marginEnd('2')} md:${direction.marginEnd('3')}`}
              />
            </div>
            <h1 className="text-lg md:text-xl font-bold">{t('appName')}</h1>
          </div>
          <h2 className={`text-2xl md:text-3xl font-bold mb-3 ${direction.textAlign}`}>{t('joinUs')}</h2>
          <p className={`text-sm md:text-base opacity-80 ${direction.textAlign}`}>{t('createAccountDescription')}</p>

          {/* Feature highlights */}
          <div className="mt-8 space-y-6">
            <div className={`flex items-start ${direction.flexDirection}`}>
              ...
              <div className={`${direction.marginStart('4')} ${direction.textAlign}`}>
        ...
        {/* Bottom content */}
        <div className={`z-10 p-6 md:p-8 lg:p-10 ${direction.textAlign}`}>
      ...
      <div className={`flex-1 flex justify-center items-start pt-4 md:pt-6 lg:pt-8 xl:pt-10 pb-4 md:pb-6 lg:pb-8 xl:pb-10 px-4 md:px-6 lg:px-8 xl:px-10 auth-container ${isRTL ? 'md:order-1' : 'md:order-2'}`}>
        <div className="w-full max-w-md md:max-w-lg lg:max-w-xl mt-0 mb-auto">
          ...
          <div className="flex md:hidden items-center justify-center mb-6 sm:mb-8">
            <div className={`flex items-center responsive-fade-in ${direction.flexDirection}`}>
              <NextImage
                src="/images/logo.svg"
                alt={t('logo')}
                width={40}
                height={40}
                className={`w-8 h-8 sm:w-10 sm:h-10 object-contain ${direction.marginEnd('2.5')} sm:${direction.marginEnd('3')}`}
              />
