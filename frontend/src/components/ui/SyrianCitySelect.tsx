...
export default function SyrianCitySelect({
  ...
  const { isRTL, textAlign, marginStart, paddingStart, paddingEnd } = useRTL();
  ...
  return (
    <div className={`group relative ${className}`}>
      ...
      {label && (
        ...
        >
          {label}
          {required && <span className={`text-red-500 ${marginStart('1')}`}>*</span>}
        </label>
      )}
      ...
      <div className="relative">
        {/* City icon */}
        <div className={`absolute inset-y-0 ${isRTL ? 'right-0' : 'left-0'} flex items-center ${paddingStart('4')} pointer-events-none`}>
        ...
        <input
          ...
          required={required}
          className={`block w-full ${paddingStart('16')} ${paddingEnd('10')} py-3.5 border-2 rounded-xl shadow-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 text-sm sm:text-base bg-white dark:bg-gray-800 text-gray-900 dark:text-white transition-all duration-300 hover:shadow-md focus:shadow-lg focus:scale-[1.02] disabled:opacity-60 disabled:cursor-not-allowed ${textAlign} ${
            error
        ...
        {/* Dropdown arrow */}
        <div className={`absolute inset-y-0 ${isRTL ? 'left-0' : 'right-0'} flex items-center ${paddingEnd('4')}`}>
          <svg
      ...
      {/* Dropdown menu */}
      {isOpen && !loading && (
        <div className="absolute z-[60] w-full mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg max-h-64 overflow-y-auto">
          {filteredCities.length === 0 ? (
            ...
            Object.entries(groupedCities).map(([governorate, governorateCities]) => {
              ...
              return (
                <div key={governorate}>
                  {/* Governorate header - only show if needed */}
                  {shouldShowHeader && (
                  ...
                  {governorateCities.map((city) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 ${textAlign} hover:bg-gray-50 dark:hover:bg-gray-700 focus:bg-gray-50 dark:focus:bg-gray-700 focus:outline-none transition-colors duration-150`}
                    >
      ...
      {/* Loading state */}
      {loading && (
        <div className={`mt-2 flex items-center text-sm text-gray-500 dark:text-gray-400 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
          <svg className={`animate-spin h-4 w-4 ${marginStart('3')}`} fill="none" viewBox="0 0 24 24">
