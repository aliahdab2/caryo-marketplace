...
export const EnhancedLoadingState = React.memo<LoadingStateProps>(({
  ...
  const renderLoadingIndicator = () => {
    switch (type) {
      ...
      case 'dots':
        return (
          <div className="flex space-x-1" role="status" aria-label={t('loading', 'Loading')}>
            {[0, 1, 2].map((i) => (
              <div
                key={i}
  ...
  return (
    <div className={`flex flex-col items-center justify-center p-4 ${className}`}>
      ...
      <span className="sr-only">{t('loading', 'Loading content, please wait')}</span>
    </div>
  );
});
  ...
  return (
    ...
    >
      ...
      <span id="retry-description" className="sr-only">
        {t('retryDescription', 'Click to retry the failed operation')}
      </span>
    </div>
  );
});
      ...
      <div className="flex items-start">
        ...
        {onClose && (
          <button
            ...
            aria-label={t('close', 'Close notification')}
          >
