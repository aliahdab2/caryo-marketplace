...
**Version:** 2.0.0 (Current Implementation — 44 migrations through V59)
**Last Updated:** March 2026
**Database:** PostgreSQL 16+ (H2 for unit tests only)
...
erDiagram
    ...
    %% Messaging (V15 restructure)
    car_listings ||--o{ conversations : about
...
**Entity Mapping:**
- Java Entity: `CarCondition`
- Maps to: `@Table(name = "car_conditions")`
...
**Entity Mapping:**
- Java Entity: `BodyStyle`
- Maps to: `@Table(name = "body_styles")`
- **Usage:** Used for grouping listings by body style (Sedan, SUV, Hatchback, etc.) in analytics endpoints like `/api/listings/counts/body-styles`

**Common Body Styles:**
- `sedan`, `suv`, `hatchback`, `coupe`, `pickup`, `convertible`, `wagon`, `van`, `minivan`, `crossover`
...
**Entity Mapping:**
- Java Entity: `FuelType`
- Maps to: `@Table(name = "fuel_types")`
...
**Entity Mapping:**
- Java Entity: `Transmission`
- Maps to: `@Table(name = "transmissions")`
...
**Entity Mapping:**
- Java Entity: `DriveType`
- Maps to: `@Table(name = "drive_types")`
...
**Entity Mapping:**
- Java Entity: `SellerType`
- Maps to: `@Table(name = "seller_types")`
...
- **Unique Constraints**: Slug uniqueness is enforced at the JPA entity level (`@Column(unique = true)`)
- **ID Type**: All reference tables use `BIGINT` (not INT) to match Java `Long` type and allow for future scalability
...
### **conversations** (V15 — replaced old messages structure)
```sql
...
### **messages** (V15 — now references conversations, not users directly)
```sql
...
## 💳 **PAYMENT & MODERATION TABLES**

### **payment_transactions** (V53)
```sql
...
## 🔍 **FULL-TEXT SEARCH** (V59)

```sql
-- Added to car_listings table
...
-- Trigger function auto-updates search_vector on insert/update
```
...
### **Current Migration Files (44 total)**

**Core Schema:**
- `V1__Initial_Schema.sql` — Core tables (users, roles, car_listings, listing_media, favorites, etc.)
- `V1_6__seed_makes_and_models.sql` — Initial car makes/models data
- `V1_36__Create_newsletter_subscriptions_table.sql`
- `V1_37__Create_saved_searches_tables.sql`
- `V1_38__Add_search_query_hash_to_saved_searches.sql`
- `V1_39__Add_email_verification_fields.sql`
- `V2__reference_data.sql` — Reference data (conditions, body styles, fuel types, etc.)

**Schema Evolution (V3–V13):**
- `V3__Add_Car_Listings_Model_Year_Index.sql`
- `V4__Add_model_id_to_car_listings.sql`
- `V6__Update_governorates_not_null.sql`
- `V7__Refactor_Geographic_Hierarchy.sql` — Geographic hierarchy (countries, governorates, locations)
- `V8__Add_denormalized_columns_to_car_listings.sql` — Brand/model/governorate name denormalization
- `V9__Populate_denormalized_fields.sql`
- `V10__Populate_denormalized_fields_after_sample_data.sql`
- `V11__Fix_brand_column_constraint.sql`
- `V12__Add_seller_type_to_users.sql`
- `V13__Add_currency_to_car_listings.sql`

**Media & Messaging (V14–V19):**
- `V14__Add_Video_Support.sql` — Video fields on listing_media
- `V15__create_messaging_tables.sql` — Conversations, message attachments, participants
- `V16__fix_partial_indexes.sql`
- `V17__fix_conversations_index.sql`
- `V18__Allow_empty_message_content.sql`
- `V19__Remove_content_check_constraint.sql`

**Dealer & Data (V20–V26):**
- `V20__Create_dealer_table.sql` — Initial dealer table
- `V21__Comprehensive_verification_method_fix.sql`
- `V25__Load_CarQuery_Data.sql` — CarQuery API data import
- `V26__Create_Data_Approval_Queue.sql`

**Features & Enhancements (V40–V59):**
- `V40__Create_Sync_Status_Table.sql`
- `V41__Add_User_Preferred_Language.sql`
- `V42__Add_Listing_Search_Prominence.sql`
- `V51__Add_trial_fields_to_dealers.sql` — Dealer trial/subscription system
- `V52__Fix_notifications_sent_json_format.sql`
- `V53__Create_payment_transactions_table.sql` — Payment system
- `V54__Create_user_reports_table.sql` — User reporting
- `V55__Create_user_blocks_table.sql` — User blocking
- `V56__Add_dealer_public_profile_fields.sql` — Dealer public profiles
- `V57__Add_image_moderation_status.sql`
- `V58__Add_token_version_to_users.sql`
- `V59__Add_Full_Text_Search.sql` — PostgreSQL tsvector/tsquery

**Repeatable Migrations:**
- `R__reference_data.sql`
- `R__sample_car_listings.sql`
- `R__sample_car_listings_new.sql`
